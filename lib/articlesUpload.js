...
var upload = {
	...
	sendToServer: function (payloadPromises) {
		...
		Q.all(payloadPromises).then(function (payloads) {
			...
			var defaultLocaleErrors = [];
			
			payloads.forEach(function (payload) {
				
				if(payload['/journalarticle/add-article'].titleMap[Config.fetch('defaultLocale')] === undefined) {
					defaultLocaleErrors.push(payload['/journalarticle/add-article'].titleMap);
				}
			...
			// A couple of line above we're checking if the titleMap returns undefined, if it does
			// it's most probably because the defaultLocale does not exist
			// in the article file (eg article file is in sv_SE instead of en_US).
			// this check should probably be done at some other place in this object.
			if (defaultLocaleErrors.length > 0) {
				utilities.writeToScreen('Error, Locale mismatch!', Constants.fetch('SEVERITY_IMPORTANT'), Constants.fetch('SCREEN_PRINT_ERROR'));
				utilities.writeToScreen('The defaultLocale (' + Config.fetch('defaultLocale') + ') does not exist in one or more of the article(s) files:', Constants.fetch('SEVERITY_IMPORTANT'), Constants.fetch('SCREEN_PRINT_ERROR'));
				console.dir(defaultLocaleErrors);
				throw Error('The defaultLocale does not exist in one or more of the article(s) files')
			}
