...
//
//var articlesFolders					= require('./ArticlesJournalFolders.js');
...
var upload = {
	...
	uploadAllArticles: function() {
		...
		Q.resolve()
			...
			.then(upload.doStructureTemplateIdMapping)
	...
	sendToServer: function (payloadPromises) {
		...
		Q.all(payloadPromises).then(function (payloads) {
			...
			//getData('[' + payloads.join() + ']').then(function (ret) {
			//	console.dir(ret);
			//});
	...
	createNewArticleUploadPromise: function () {
		...
		return localAllArticles.map(function (article) {
			...
			utilities.xmlMapToObj(article.title, 'Title', true)
				...
				.then(function (){
					uploadObj.groupId = article.ddmToolServerGroupId;
					...
					uploadObj['serviceContext.scopeGroupId'] = article.ddmToolServerGroupId;

					deferred.resolve({'/journalarticle/add-article': uploadObj });
					
					//console.dir(article.title);
					//console.dir(uploadObj.titleMap);
	...
	doStructureTemplateIdMapping: function () {
		...
		localAllArticles = localAllArticles.map(function (article) {
			article.ddmToolServerGroupId = Sites.getSingleValue('friendlyURL', article.ddmToolGroupFriendlyURL, 'groupId');
