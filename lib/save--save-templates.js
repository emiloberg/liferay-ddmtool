...
var saveTemplates = function(e) {
	...
	if(countAlreadyUpToDate.length > 0) {
		if (countAlreadyUpToDate.length === e.length) {
			utilities.writeToScreen('\nAll ' + countAlreadyUpToDate.length + ' templates already up to date', Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_HEADING'));
		} else {
			utilities.writeToScreen('\n' + countAlreadyUpToDate.length + ' templates - ' + states[0].heading, Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_HEADING'));
		}
	}
