...
var saveTemplates = function(e) {
	...
	if(countAlreadyUpToDate.length > 0) {
		utilities.writeToScreen(countAlreadyUpToDate.length + ' templates - ' + states[0].heading, Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_HEADING'));
	}
	...
	for (var i = 1; i < states.length; i++) {
		...
		if (outArr.length > 0) {
			utilities.writeToScreen(outArr.length + ' templates - ' + states[i].heading, Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_HEADING'));
			outTable = new Table({
				head: ['Name', 'Type'],
				chars: {
