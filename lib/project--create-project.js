	...
	inquirer.prompt( questionsProject, function(answersProject) {
		...
		function askForHosts() {
			inquirer.prompt(questionsHosts, function(answersHosts) {
				...
				utilities.writeToScreen('Testing connection', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_INFO);
				getData('{"/portal/get-build-number": {}}', true, answersHosts.host, answersHosts.username, answersHosts.password)
					.then(function () {
						utilities.writeToScreen('Connection okay!', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_INFO);
						// Resolve Promise if connection Works
						deferred.resolve();
					}, function (e) {
						// If connection didn't work, ask if user want's to save it anyway.
						utilities.writeToScreen('Could not establish connection (' + e + ')', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_ERROR);
						...
						inquirer.prompt([{
							...
							if (answers.proceed === 'reenter') {
								...
								utilities.writeToScreen('', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_INFO);
								utilities.writeToScreen('Previously entered values within parentheses\nJust press <enter> if you want to leave the field unchanged', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_INFO);
								utilities.writeToScreen('', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_INFO);
					...
					.done(function () {
						...
						inquirer.prompt([{
							...
							if (answersAskAgain.askAgain) {
								...
								utilities.writeToScreen('', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_SAVE);
								utilities.writeToScreen('Project created!', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_SAVE);
								utilities.writeToScreen('Settings saved to ' + Fixed.settingsFolder + '/' + Fixed.projectsFolder + '/' + answersProject.projectName.toLowerCase() + '.json', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_SAVE);
								utilities.writeToScreen('', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_SAVE);

								var router = require('./router.js');
								router(Constants.STEP_JUST_CREATED_PROJECT);
