...
var saveStructures = function(e, filesRootPath) {
	var filePath;
	var fileContent;
	...
	if(countAlreadyUpToDate.length > 0) {
		utilities.writeToScreen(countAlreadyUpToDate.length + ' structures - ' + states[0].heading, Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_SAVE'));
	}
	...
	for (var i = 1; i < states.length; i++) {
		...
		if (outArr.length > 0) {
			utilities.writeToScreen(outArr.length + ' structures - ' + states[i].heading, Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_SAVE'));
