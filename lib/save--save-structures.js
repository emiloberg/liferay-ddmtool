...
var saveStructures = function(e) {
	...
	if(countAlreadyUpToDate.length > 0) {
		if (countAlreadyUpToDate.length === e.length) {
			utilities.writeToScreen('\nAll ' + countAlreadyUpToDate.length + ' structures already up to date', Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_HEADING'));
		} else {
			utilities.writeToScreen('\n' + countAlreadyUpToDate.length + ' structures - ' + states[0].heading, Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_HEADING'));
		}
	}
	...
	for (var i = 1; i < states.length; i++) {
		...
		if (outArr.length > 0) {
			utilities.writeToScreen('\n' + outArr.length + ' structures - ' + states[i].heading + '\n', Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_HEADING'));
