...
var createUploadObject = function (file) {
	...
	if (isNewDDM) {
		if (returnObj.fileClassObj.clazz === 'com.liferay.portlet.dynamicdatamapping.model.DDMStructure') {
			...
			// New ADT Template
			var createUploadObjectNewGenericTemplate = require('./upload--co-new-template.js');
			createUploadObjectNewGenericTemplate(newScript, fileClassObj, fileName, fileLanguageType, isNewDDM, returnObj).then(function (resp) {
				deferred.resolve(resp);
			}, function (er) {
				deferred.reject(er);
			});
