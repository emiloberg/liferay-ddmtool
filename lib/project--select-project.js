...
var selectProject = function () {
	...
	if (RouterArgs.fetch('hasProject')) {
		...
		var projectSettingsPath = Constants.fetch('settingsFolder') + '/' + Constants.fetch('projectsFolder') + '/' + RouterArgs.fetch('projectName').toLowerCase() + '.json';
		if (fs.existsSync(projectSettingsPath)) {
			var loadProject = require('./project--load-project.js');
			loadProject(RouterArgs.fetch('projectName').toLowerCase() + '.json'); //TODO ÄNDRA HÄR.
		} else {
		...
		var globOptions = {
			cwd: Constants.fetch('settingsFolder') + '/' + Constants.fetch('projectsFolder')
		};
