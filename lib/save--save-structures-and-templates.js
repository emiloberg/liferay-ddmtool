...
var saveStructuresAndTemplatesToFile = function(e) {
	...
	var idDLFileEntryMetadata = LrClassNameConfig.getSingleValue('clazz', 'com.liferay.portlet.documentlibrary.model.DLFileEntryMetadata', 'id');
	var friendlyName;
...
//			filePath = Config.filesFolder + '/' + utilities.getFilesPathFromClassNameId(e[i].classNameId);
		...
		if (e[i].hasOwnProperty('storageType') && e[i].hasOwnProperty('xsd')) {
			...
			if (e[i].classNameId === idDLFileEntryMetadata) {
				if (e[i].type === 1) {
					filePath = filePath + '/' + Constants.fetch('pathSlugDocumentTypes') + '/' + e[i].nameCurrentValue + '.' + e[i].storageType;
				} else {
					filePath = filePath + '/' + Constants.fetch('pathSlugMetadataSets') + '/' + e[i].nameCurrentValue + '.' + e[i].storageType;
				}
			} else {
