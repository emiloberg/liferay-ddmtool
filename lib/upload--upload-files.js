	...
	Q.all(preparePayloadPromises).then(function (uploadObjects) {
		...
		for (var x = 0; x < states.length ; x++) {
			...
			if (states[x].table.length > 0) {
				utilities.writeToScreen('', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_HEADING);
				utilities.writeToScreen(states[x].heading + ' (' + states[x].table.length + ')', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_HEADING);
				utilities.writeToScreen('', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_HEADING);
				utilities.writeToScreen(states[x].table.toString(), Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_INFO);
				utilities.writeToScreen('', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_HEADING);
			}
			...
			inquirer.prompt([
					...
					if (answers.confirm === true) {
						...
						getData('[' + fullPayload.join() + ']').then(function (resp) {
							...
							utilities.writeToScreen('Files updated/created!', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_SAVE);
							router(Constants.STEP_JUST_UPLOADED_DDMS);
						}, function (e) {
							console.dir(e);
