...
var uploadFiles = function (files, options) {
	var Config							= require('./SingleConfig.js');


	// Set default options and save to uploadOptions object.
	options = typeof options !== 'undefined' ? options : {};
	options.watchMode = typeof options.watchMode !== 'undefined' ? options.watchMode : false;
...
var doUploads = function (uploadObjects) {
	...
	if (!uploadOptions.watchMode === true) {
	...
	if (states[1].table.length > 0 || states[2].table.length > 0 ) {
		...
		if (uploadOptions.watchMode === true) {
			getData('[' + fullPayload.join() + ']').then(function (resp) {
				...
				var updatedDDMsNames = [];
				resp.forEach(function(entry) {
				    updatedDDMsNames.push((entry.nameCurrentValue));
				});
				...
				utilities.writeToScreen('Upload successful! (' + updatedDDMsNames.join() + ')', Constants.fetch('SEVERITY_NORMAL'), Constants.fetch('SCREEN_PRINT_SAVE'), true);
