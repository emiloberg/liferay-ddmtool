...
var download = {
	...
	downloadAllFromServer: function () {
		...
		utilities.writeToScreen('Getting data from server', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_INFO);
	...
	getClassNameIds: function () {
		utilities.writeToScreen('Downloading id\'s', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_INFO);
		...
		return getData('[' + payload.join() + ']').then(
			function (e) {
				...
				cache.saveToCache(LrClassNameConfig, Fixed.cacheClassNameIdsByName);
	...
	getUserSites: function() {
		utilities.writeToScreen('Downloading list of sites', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_INFO);
	...
	utilities.writeToScreen('Downloading company site', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_INFO);
	return getData('{"/group/get-company-group": {"companyId": "' + globalCompanyId + '"}}').then(
		function (e) {
			...
			cache.saveToCache(globalSites, Fixed.cacheSitesFilename);
		});
	},


	getStructuresFromListOfSites: function () {
		utilities.writeToScreen('Downloading structures', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_INFO);
		...
		return getData('{"/ddmstructure/get-structures": {"groupIds": [' + sitesList.join() + ']}}').then(
			function (e) {
				...
				cache.saveToCache(globalSitesWithStructure, Fixed.cacheSitesWithStructuresFilename);
	...
	getTemplates: function () {
		...
		utilities.writeToScreen('Downloading templates', Constants.SEVERITY_NORMAL, Constants.SCREEN_PRINT_INFO);
		var payload = [];
