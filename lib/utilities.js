...
var utilities = {
    ...
    xmlMapToObj: function (xml, type, allowAllCharacters){

        allowAllCharacters = typeof allowAllCharacters !== 'undefined' ? allowAllCharacters : false;
    ...
    filenameIsNotBlacklisted: function (file) {
        var blacklistPattern = Config.fetch('watchIgnorePattern');
        if (blacklistPattern !== undefined) {
            var filename = utilities.filenameAndPathToFilename(file, true);
            var re = new RegExp(blacklistPattern);
            var shouldBeIgnored = re.test(filename);
    ...
    timestampToInt: function (timestamp) {
        if (timestamp === null) {
            return {
                izNull: true,
                int: {
