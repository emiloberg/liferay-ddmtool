...
var coTemplate = function (file) {
	...
	try {
		...
		returnObj.exception = 'Could not read file ' + file;
	...
	// If we're updating a template. Check if it actually needs updating or if it's already up to date.
	if (newObj.isUpdatingDDM) {
		if(newObj.oldObj.script === newObj.newScript) {
			newObj.status = 'uptodate';
